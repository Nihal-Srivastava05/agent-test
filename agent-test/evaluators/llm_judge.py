...
class LLMJudgeEvaluator(BaseEvaluator):
    ...
    def _evaluate_single_case(self, data: Dict[str, Any]) -> EvaluationResult:
        ...
        criteria = data.get("evaluation_criteria", self._get_config_value("criteria", ["accuracy", "relevance"]))
    ...
    def _create_evaluation_prompt(
        ...
        for criterion in criteria:
            prompt_parts.append(f"- {criterion}: {self._get_criterion_description(criterion)}")
    ...
    def _get_openai_judgment(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            response = self.llm_client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert AI evaluator. Provide evaluations in valid JSON format."},
    ...
    def _get_gemini_judgment(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            full_prompt = "You are an expert AI evaluator. Provide evaluations in valid JSON format. " + prompt
    ...
    def _parse_fallback_response(self, content: str) -> Dict[str, Any]:
        ...
        if "pass" in content_lower and "fail" not in content_lower:
            passed = True
            score = 0.8
        elif "fail" in content_lower or "incorrect" in content_lower or "wrong" in content_lower:
            passed = False
            score = 0.2
        else:
            # Neutral case
