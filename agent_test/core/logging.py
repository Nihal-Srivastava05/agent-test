...
class AgentTestLogger:
    ...
    def log(self, level: str, category: str, message: str, 
            details: Dict[str, Any] = None, test_name: str = None):
    ...
    def info(self, category: str, message: str, **kwargs):
        ...
        self.log("INFO", category, message, kwargs.get("details"), kwargs.get("test_name"))
    
    def warning(self, category: str, message: str, **kwargs):
        """Log warning level message."""
        self.log("WARNING", category, message, kwargs.get("details"), kwargs.get("test_name"))
    
    def error(self, category: str, message: str, **kwargs):
        """Log error level message."""
        self.log("ERROR", category, message, kwargs.get("details"), kwargs.get("test_name"))
    
    def debug(self, category: str, message: str, **kwargs):
        """Log debug level message (only if verbose)."""
        if self.verbose:
            self.log("DEBUG", category, message, kwargs.get("details"), kwargs.get("test_name"))
    ...
    def test_completed(self, test_name: str, passed: bool, duration: float, 
                      score: Optional[float] = None, error: str = None):
        """Log test completion."""
        ...
        if passed:
            self.info("TEST_PASS", f"✅ {test_name}", details=details, test_name=test_name)
        else:
            self.error("TEST_FAIL", f"❌ {test_name}", details=details, test_name=test_name)
    ...
    def evaluator_started(self, evaluator_name: str, test_name: str):
        ...
        if self.verbose:
            self.debug("EVALUATOR", f"Running {evaluator_name} for {test_name}", 
                      test_name=test_name)
    
    def evaluator_completed(self, evaluator_name: str, test_name: str, 
                           passed: bool, score: Optional[float] = None, error: str = None):
        """Log evaluator completion."""
        details = {"score": score, "error": error}
        
        if passed:
            if self.verbose:
                self.debug("EVALUATOR_PASS", f"✅ {evaluator_name}: PASSED", 
                          details=details, test_name=test_name)
        else:
            if self.verbose:
                self.debug("EVALUATOR_FAIL", f"❌ {evaluator_name}: FAILED", 
                          details=details, test_name=test_name)
    
    def discovery_started(self, path: str, pattern: str):
        """Log test discovery start."""
        self.info("DISCOVERY", f"🔍 Discovering tests in {path} with pattern '{pattern}'")
    
    def discovery_completed(self, test_count: int, files_scanned: int):
        """Log test discovery completion."""
        self.info("DISCOVERY", 
                 f"📋 Found {test_count} tests in {files_scanned} files")
    ...
    def _display_final_summary(self, summary: Dict[str, Any], total_time: float):
        ...
        if self.verbose:
            ...
            avg_duration = summary.get('total_duration', 0) / max(summary.get('total_tests', 1), 1)
